...
integer :: i,iii,j,jjj,step ! indices for loops
...
integer :: prev(2),prev_der(10)
integer :: modifier ! = 0 init, 1 up, 2 down
integer :: maximum,minimum
real*8 :: mean,sigma
character*32 :: file1='prev_max.dat',file2='prev_min.dat'
NAMELIST/DADES/x,lambda,mutation_rate,SEED0,NSEED,time_max,time_inc
! ---------------------------------------------------------------
! Read input file
open(unit=11,file='../inp/sir_mut.inp',status='old')
!open(unit=11,file='sir_mut.inp',status='old') ! parallel
read(11,DADES)
close(11)
lambda = lambda/100d0
...
if (mod(step,100).eq.0) then
    call prevalence(n_infected,N,time,prev,prev_der,modifier,minimum,maximum)
    !write(35,*) prev
    !write(35,*) prev_der
endif
print*, time, dble(n_infected)/dble(N), n_mutation, immune_length
...
print*, mutation_rate, int(dble(seed-seed0+1)/dble(nseed)*1d2),'%',t_actual-t_start,&
    & (t_actual-t_start)*(dble(nseed)/dble(seed-seed0+1)-1d0)
! ===================
enddo loop_seed
! ===================
close(35)
call running_variance(file1,mean,sigma)
inquire(file='prev_max_lambda.dat', exist=exist)
if(.not.exist) call execute_command_line('touch prev_max_lambda.dat')
open(35,file='prev_max_lambda.dat',access='append',status='old')
write(35,*) lambda,mutation_rate,mean,sigma

close(36)
call running_variance(file2,mean,sigma)
inquire(file='prev_min_lambda.dat', exist=exist)
if(.not.exist) call execute_command_line('touch prev_min_lambda.dat')
open(36,file='prev_min_lambda.dat',access='append',status='old')
write(36,*) lambda,mutation_rate,mean,sigma
