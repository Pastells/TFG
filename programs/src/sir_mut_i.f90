...
! ------------------------------------------------------------
! seed loop
!open(30,file='mutation_time_prevalence.dat')
...
!write(FMT,'("(F8.4,",I0,"(5X,F7.4))")') infected_i_length+1
!print(FMT), time,dble(n_infected)/dble(N),dble(n_infected_i)/dble(N)
! ===================
...
!print*, time, dble(n_infected)/dble(N), n_mutation, immune_length, dble(n_infected_i)/dble(N)
!write(FMT,'("(F8.4,I3,",I0,"(5X,F7.4))")') infected_i_length+1
n_active_mutations = 0
do i=1,n_mutation
    if (n_infected_i(i).ge.1)  n_active_mutations =  n_active_mutations + 1
enddo
!print(FMT), time,dble(n_infected)/dble(N),dble(n_infected_i)/dble(N)
print('(F8.4,5X,I5,5X,F8.4)'), time,n_active_mutations,dble(n_infected)/dble(N)
time = time + poisson_time(lambda_sum,idum)
! ===================
enddo loop_time
!do i=2,n_mutation
    !write(30,*) i,mutation_time_prevalence(i,:),n_recovered_i(i),seed
!enddo
enddo loop_seed
! ===================
!close(30)
write(name,'("./prevalence > prevalence_",I3.3,".dat")') int(lambda*100d0)
call execute_command_line(name)
!write(name,'("mv mutation_time_prevalence.dat mutation_time_prevalence_",I3.3,".dat")') int(lambda*100d0)
!call execute_command_line(name)
! *******************************************************************
end program main
