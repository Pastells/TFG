...
real*8 :: lambda
! probability of R -> S
real*8 :: beta, beta_func
real*8 :: lambda_sum, prob_heal, prob_infect
real*8 :: poisson_time, time, time_max
real*8 :: M_var, M_var_1, S_var, mean=0d0
real*8 :: ran2, random
NAMELIST/DADES/x,lambda,beta,SEED0,NSEED,time_max
! ------------------------------------------------------------------
! Read input file
open(unit=11,file='../inp/sirs.inp',status='old')
read(11,DADES)
close(11)
lambda = lambda/100d0
beta = beta/100d0
...
!loop_seed: do SEED = SEED0-NSEED+1,SEED0
    !print*, dble(seed-seed0)/dble(nseed),'%'
    idum = -seed
!-------
...
!                                      |_|
time = 0d0
print*, time, dble(n_infected)/dble(N), n_infected, n_active_edges, n_recovered, 0d0
! ===================
loop_time: do while ( (n_infected.gt.0) .and. (time.lt.time_max) )
! ===================
beta = beta_func(n_recovered,N)
lambda_sum = dble(n_infected)+dble(n_active_edges)*lambda+beta*dble(n_recovered)
...
print*, time, dble(n_infected)/dble(N), n_infected, n_active_edges, n_recovered, beta
! ===================
enddo loop_time
! ===================
!if (x.gt.1) then
...
!elseif (x.eq.1) then
    !if (time.gt.1d0)  survival = survival + 1
!endif
! ===================
enddo loop_seed
! ===================
...
real*8 function beta_func(n_recovered,N)
implicit none
integer :: n_recovered,N
beta_func=tanh( dble(n_recovered)/dble(N) )
return
